...
//1) - Criar grid com lista de campos
//2) - Selecionar quais campos serão incluidos no form.
//3) - Gerar form para cadastro e edição.
//4) - Criar metodoa para exclusão.
//5) - Criar grid conforme tabela com busca e filtro.
//6) criar grid por "select"

 
//infomarções sobre os campos da tabela.

$info_campos = $mysql->GetInfoFieldsTable($tabela);

//lista dos campos da tabela a serem apresentados.
$show_info_campos = null;
...
function CamposDBTable($info_campos,$show_info_campos){
global $mysql;
global $tabela;
    foreach ($info_campos as $key => $value) {
        ...
        foreach ($value as $key2 => $value2) {
            if ($key2 == 'Field') {
                if (is_null($show_info_campos) || (!array_key_exists($value2, $show_info_campos))) {
                    $campo = $value2;
                }
            }
        ...
        $show_info_campos[$campo] = $temp;
    }
    return $show_info_campos;
}

function CamposToHtmlInput($show_info_campos){
    $html = '';
    foreach ($show_info_campos as $key => $value) {
...
function CamposMsgValidacaoMascara($show_info_campos){
    foreach ($show_info_campos as $key => $value) {
...
function CamposToListBox($campos){
...
function PaginaMontaForm($info_campos,$show_info_campos){
global $info_campos;
global $show_info_campos;
$posDB = CamposDBTable($info_campos,$show_info_campos); 
$js = MontarJSPagina();    
...
function GeraPaginaHTML($info_campos,$show_info_campos){
    global $tabela;
...
function GeraCriaPaginaHTML(){
global $info_campos;
global $show_info_campos;

$CamposDB = CamposDBTable($info_campos,$show_info_campos);     
...
//$saida = '<?php ';
$saida = GeraCriaPaginaHTML($info_campos,$show_info_campos);
//$saida = PaginaMontaForm($info_campos,$show_info_campos);
//$saida .= '?/>';
